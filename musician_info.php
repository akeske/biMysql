...
<?php
// python sqlmap.py -u "http://127.0.0.1/biTempMysql/musician_info.php?id=70&mus_id=2" --dbs --threads=5 --eta --batch --is-dba -v 2 --technique=BEUST -t "/home/user/Έγγραφα/aa.txt"  --fresh-queries > "/home/user/Έγγραφα/after.txt"
// python sqlmap.py -u "http://127.0.0.1/biTempMysql/musician_info.php?id=70&mus_id=2" --threads=5 --batch --technique=BEUST --tables -D conservatory --fresh-queries > "/home/user/Έγγραφα/after_tables.txt"
// python sqlmap.py -u "http://127.0.0.1/biTempMysql/musician_info.php?id=70&mus_id=2" --threads=5 --batch --technique=BEUST --columns -D conservatory -T user --fresh-queries > "/home/user/Έγγραφα/after_table_user_fields.txt"
// python sqlmap.py -u "http://127.0.0.1/biTempMysql/musician_info.php?id=70&mus_id=2" --threads=5 --batch --technique=BEUST --dump -D conservatory -T user --fresh-queries > "/home/user/Έγγραφα/after_table_user_dump.txt"
...
if(isset($_GET['id'])){
	...
	if(preg_match($pattern, $_GET['mus_id'], $matches, PREG_OFFSET_CAPTURE) ||
...
<td>
	<form method="post" action="musician_info.php?id=<?php echo $row['id']; ?>&mus_id=<?php echo $row['musician_id']; ?>" name="editMucisianForm" class="pure-form">
		...
		<legend>Insert new info about musician</legend>
			<input disabled value="<?php if(isset($_GET['id'])){ echo $mus_id; } ?>" size="1" type="text" autocomplete="off" maxlength="12"/>
			<input disabled value="<?php if(isset($_GET['id'])){ echo $name; } ?>" size="14" type="text" autocomplete="off" maxlength="30"/>
			<input value="" id="new_telephone" pattern="[a-zA-Z0-9\s\u00a1-\uffff]{2,12}" size="14" type="text" name="new_telephone" autocomplete="off" placeholder="New Telephone" maxlength="12"/>
